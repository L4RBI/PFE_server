...
def get_cameraman_tensor(sidelength, path):
    img = Image.open(path)
    transform = Compose([
        Resize((sidelength)),
        ToTensor(),
...
class ImageFitting(Dataset):
    def __init__(self, sidelength, path):
        super().__init__()
        img = get_cameraman_tensor(sidelength, path)
...
img = sys.argv[1]
cameraman = ImageFitting(
    256, img)
dataloader = DataLoader(cameraman, batch_size=1,
...
model_input, ground_truth = model_input.cuda(), ground_truth.cuda()
model_output, coords = img_siren(model_input)
cv.imwrite(sys.argv[2], model_output.cpu().view(512,
                                                512).detach().numpy()*127.5+127.5)
print("done")
